...
$('.klarna-checkout-main').load('index.php?routes=extension/payment/klarna_checkout/main', {response: 'template'}, function(data) {
	window._klarnaCheckout(function(api) {
		addSidebarOverlay();
		api.suspend();

		$('.klarna-checkout-sidebar').load('index.php?routes=extension/payment/klarna_checkout/sidebar', function() {
			api.resume();
		...
		api.on({
			shipping_address_change: function(data) {
				...
				$.post('index.php?routes=extension/payment/klarna_checkout/shippingAddress', data, function(data) {
					$('.klarna-checkout-sidebar').load('index.php?routes=extension/payment/klarna_checkout/sidebar');

					$.get('index.php?routes=extension/payment/klarna_checkout/cartTotal', function(total) {
						setTimeout(function() {
