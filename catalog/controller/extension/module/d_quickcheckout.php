	...
	public function index(){
		...
		if($this->validate()) {		
			...
			if($this->settings['general']['enable']){
				...
				return $this->load->view('extension/module/d_quickcheckout', $this->data);
			} //if enabled
		}else{
			...
			return $this->load->view('d_quickcheckout/empty', $this->data);
		}
	}
 ...
 */
	private function get_payment_address_view(){
		...
		//URBAN SHIPPING//
		// if($this->config->get('urban_shipping_status') && $this->config->get('chili_setting_domain')){ 
			$data['fields']['city']['error']=array(array('not_empty'=>1,'text'=>'error_city'));
			$data['fields']['city']['type']='select';
			$data['fields']['city']['refresh']='4';
			$data['fields']['city']['sort_order']='9';
		// }
		//END URBAN SHIPPING//
		...
		// URBAN SHIP //
		// if ($this->config->get('urban_shipping_status')) {
			$data['fields']['city']['options'] = $this->get_district_by_zone_id($data['fields']['zone_id']['value']);
		// }
		// END URBAN SHIP //
 ...
 */
	...
	// URBAN SHIPPING//
		private function get_district_by_zone_id($zone_id){
			$this->load->model('localisation/district');
			$districts =  $this->model_localisation_district->getDistrictsByZoneId($zone_id);
			$options = array();
			foreach ($districts as $district){
				$district['value'] = $district['zone_district_id']; 
				unset($district['zone_id']);
				$options[] = $district;
			}
			return $options;
		}
		...
		public function zone() {
			...
			$this->load->model('localisation/district');
				$json = array(
					...
					'name'              => $zone_info['name'],
					'city'              => $this->model_localisation_district->getDistrictsByZoneId($this->request->get['zone_id']),
