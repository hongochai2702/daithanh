<?php
class ControllerProductSearchsev extends Controller {
	public function index() {
		...
		if (isset($this->request->get['categorysev_id'])) {
			$categorysev_id = $this->request->get['categorysev_id'];
		} else {
			$categorysev_id = 0;
		}

		if (isset($this->request->get['sub_categorysev'])) {
			$sub_categorysev = $this->request->get['sub_categorysev'];
		} else {
			$sub_categorysev = '';
		}
		...
		$data['text_categorysev'] = $this->language->get('text_categorysev');
		$data['text_sub_categorysev'] = $this->language->get('text_sub_categorysev');
		...
		// 3 Level Category Searchsev
		$data['categories'] = array();

		$categories_1 = $this->model_catalog_categorysev->getCategories(0);

		foreach ($categories_1 as $categorysev_1) {
			$level_2_data = array();

			$categories_2 = $this->model_catalog_categorysev->getCategories($categorysev_1['categorysev_id']);

			foreach ($categories_2 as $categorysev_2) {
				$level_3_data = array();

				$categories_3 = $this->model_catalog_categorysev->getCategories($categorysev_2['categorysev_id']);

				foreach ($categories_3 as $categorysev_3) {
					$level_3_data[] = array(
						'categorysev_id' => $categorysev_3['categorysev_id'],
						'name'        => $categorysev_3['name'],
					);
				}

				$level_2_data[] = array(
					'categorysev_id' => $categorysev_2['categorysev_id'],
					'name'        => $categorysev_2['name'],
					'children'    => $level_3_data
				);
			}

			$data['categories'][] = array(
				'categorysev_id' => $categorysev_1['categorysev_id'],
				'name'        => $categorysev_1['name'],
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			$filter_data = array(
				...
				'filter_sub_categorysev' => $sub_categorysev,
				'sort'                => $sort,
				'order'               => $order,
				'start'               => ($page - 1) * $limit,
				'limit'               => $limit
			...
			foreach ($results as $result) {
				...
				$data['services'][] = array(
					...
					'href'        => $this->url->link('product/service', 'service_id=' . $result['service_id'] . $url)
				);
			}
			...
			if (isset($this->request->get['search']) && $this->config->get('config_customer_search')) {
				...
				$search_data = array(
					'keyword'       => $search,
					'categorysev_id'   => $categorysev_id,
					'sub_categorysev'  => $sub_categorysev,
				...
				$this->model_account_search->addSearchsev($search_data);
			}
		}

		$data['search'] = $search;
		$data['description'] = $description;
		$data['categorysev_id'] = $categorysev_id;
		$data['sub_categorysev'] = $sub_categorysev;
