...
class ModelXformXform extends Model
{
 ...
 public function validateForm($formId,$data, $isAdmin=false){
	...
	elseif($field['field_type']=='text' || $field['field_type']=='email' || $field['field_type']=='paragraph' || $field['field_type']=='price' || $field['field_type']=='dropdown')
	   {
 ...
 public function renderForm($formId,$data=array(), $isAdmin=false, $fromModule=false){
	 ...
	 foreach ($fields as $field ) {
		   ...
		   if($field['field_type']=='submit'){
		    ...
		    if($field['field_type']=='date'){
				...
				$element_html   .= '<input placeholder="'.$this->language->get('text_xform_day').'" id="field-'.$field['cid'].'" class="'.$css_class.' day" name="data['.$field['cid'].'][day]" value="'.(isset($data[$field['cid']]['day'])?$data[$field['cid']]['day']:'').'" type="text" /> / '; 
				$element_html   .= '<input placeholder="'.$this->language->get('text_xform_moth').'" class="'.$css_class.' month" name="data['.$field['cid'].'][month]" value="'.(isset($data[$field['cid']]['month'])?$data[$field['cid']]['month']:'').'" type="text" /> / '; 
				$element_html   .= '<input placeholder="'.$this->language->get('text_xform_year').'" class="'.$css_class.' year" name="data['.$field['cid'].'][year]" value="'.(isset($data[$field['cid']]['year'])?$data[$field['cid']]['year']:'').'" type="text" />'; 
				$element_html   .='</div>';
			}
			...
			if($field['field_type']=='time'){
				...
				$element_html   .= '<input placeholder="'.$this->language->get('text_xform_h').'" id="field-'.$field['cid'].'" class="'.$css_class.' hour" name="data['.$field['cid'].'][hour]" value="'.(isset($data[$field['cid']]['hour'])?$data[$field['cid']]['hour']:'').'" type="text" />&nbsp;:&nbsp;'; 
				$element_html   .= '<input placeholder="'.$this->language->get('text_xform_m').'" class="'.$css_class.' minute" name="data['.$field['cid'].'][minute]" value="'.(isset($data[$field['cid']]['minute'])?$data[$field['cid']]['minute']:'').'" type="text" /> '; 
		  ...
		  if($theme=='custom') {	
		   ...
		   if(in_array($field['cid'],$this->formError)) {
		     ...
		     if($theme=='custom') {	
			    $keyword[] = $formErrorMessage[$field['field_type']]; 
			  } else {
			    
			    $output   .= '<span class="error-block">';
			    $output   .= $formErrorMessage[$field['field_type']]; 
				$output   .= '</span>'; 
  ...
  public function getRecordData($formId,$recordId){
   ...
   public function getFormLang ($formId, $languageId='') {
      ...
      return array('labels'=>$labels,'options'=>$options,'guidelines'=>$guidelines);
