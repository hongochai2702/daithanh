...
$('select[name="store_id"]').on('change', function(e) {
	$.ajax({
		url: 'index.php?routes=design/theme/path&token=<?php echo $token; ?>&store_id=' + $('select[name="store_id"]').val(),
		dataType: 'json',
		beforeSend: function() {
...
$('#path').on('click', 'a.directory', function(e) {
	...
	$.ajax({
		url: 'index.php?routes=design/theme/path&token=<?php echo $token; ?>&store_id=' + $('select[name="store_id"]').val() + '&path=' + $(node).attr('href'),
		dataType: 'json',
		beforeSend: function() {
...
$('#path').on('click', 'a.file',function(e) {
	...
	if (!$('#tab-' + tab_id).length) {
		$.ajax({
			url: 'index.php?routes=design/theme/template&token=<?php echo $token; ?>&store_id=' + $('select[name="store_id"]').val() + '&path=' + $(node).attr('href'),
			dataType: 'json',
			beforeSend: function() {
				$(node).find('i').removeClass('fa-arrow-right');
				$(node).find('i').addClass('fa-circle-o-notch fa-spin');
...
$('.tab-content').on('click', '.btn-primary', function(e) {
	...
	$.ajax({
		url: 'index.php?routes=design/theme/save&token=<?php echo $token; ?>&store_id=' + $('.tab-content .active input[name="store_id"]').val() + '&path=' + $('.tab-content .active input[name="path"]').val(),
		type: 'post',
...
$('.tab-content').on('click', '.btn-danger', function(e) {
	if (confirm('<?php echo $text_confirm; ?>')) {
		...
		$.ajax({
			url: 'index.php?routes=design/theme/reset&token=<?php echo $token; ?>&store_id=' + $('.tab-content .active input[name="store_id"]').val() + '&path=' + $('.tab-content .active input[name="path"]').val(),
			dataType: 'json',
			beforeSend: function() {
				$(node).button('loading');
			},
