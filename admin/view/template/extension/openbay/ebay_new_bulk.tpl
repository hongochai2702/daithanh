...
<script type="text/javascript">
  ...
  function modifyPrices(id) {
      ...
      $.ajax({
          url: 'index.php?routes=extension/openbay/ebay_profile/profileGet&token=<?php echo $token; ?>&ebay_profile_id='+$('#generic_profile_'+id).val(),
          type: 'GET',
          async: true,
          dataType: 'json',
  ...
  function removeBox(id) {
      ...
      if ($('.listingBox').length == 1) {
          window.location = "index.php?routes=extension/openbay/items&token=<?php echo $token; ?>";
      } else {
          $('#activeItems').text($('#total-items').val());
      }
  }
  ...
  function itemFeatures(category_id, id) {
    ...
    var product_id = $('#product-id-' + id).val();
      ...
      $.ajax({
          url: 'index.php?routes=extension/openbay/ebay/getEbayCategorySpecifics&token=<?php echo $token; ?>&category_id=' + category_id + '&product_id=' + product_id,
          type: 'GET',
          dataType: 'json',
          beforeSend: function() { addCount(); },
          success: function(data) {
  ...
  $('#button-verify').bind('click', function() {
      ...
      $.each($('.product_id'), function(i) {
          ...
          $.ajax({
              url: 'index.php?routes=extension/openbay/ebay/verifyBulk&token=<?php echo $token; ?>&i='+id,
              type: 'POST',
              dataType: 'json',
              data: processedData,
              beforeSend: function() { addCount(); },
  ...
  $('#button-submit').bind('click', function() {
      ...
      if (confirm_box) {
          ...
          $.each($('.product_id'), function(i) {
              ...
              $.ajax({
                url: 'index.php?routes=extension/openbay/ebay/listItemBulk&token=<?php echo $token; ?>&i='+id,
                type: 'POST',
                dataType: 'json',
