...
elFinder.prototype._options = {
	...
	uiOptions : {
		...
		toolbar : [
                        ...
                        /*'home', 'up', 'rename']*/
                        ['back', 'forward','open', 'download','copy', 'cut', 'paste','rm','netmount','quicklook','view', 'sort'], 
                        ['extract', 'archive','duplicate', 'edit', 'resize','mkdir', 'mkfile', 'upload','getfile','insertfiles','mutiimage'],
                        ['search']
		],
	...
	fm.open(function(e) {
		...
		if (nativeHistory) {
			if (! nativeHistory.state) {
				nativeHistory.replaceState({thash: cwd}, null, location.pathname + location.search);
			} else {
				nativeHistory.state.thash != cwd && nativeHistory.pushState({thash: cwd}, null, location.pathname + location.search);
			}
		}
	})
	.reload(reset);
