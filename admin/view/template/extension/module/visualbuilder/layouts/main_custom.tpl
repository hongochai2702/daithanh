...
<?php  foreach($mb_position as $keys=>$items){ ?>
    <?php if($items['boss']=='theme_header' || $items['boss']=='theme_footer') { ?>
        <div class="row">
        <?php foreach($items['position'] as $key=>$item) { ?>
                <?php 
                    $position=$item["position_id"]; 
                    $position_text=$item["position_name"]; 
                ?>
                <div class="col-sm-12">
