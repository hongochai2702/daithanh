...
// Login to the API
$.ajax({
	url: '<?php echo $store_url; ?>index.php?routes=api/login',
	type: 'post',
	dataType: 'json',
...
$('#cancel-plan').on('click', function(e) {
	...
	$.ajax({
		url: 'index.php?routes=extension/payment/laybuy/cancel&token=<?php echo $token; ?>&id=<?php echo $id; ?>&source=extension',
		type: 'post',
		dataType: 'json',
		cache: false,
		beforeSend: function() {
		...
		success: function(json) {
			...
			if (json['success']) {
				if (token) {
					...
					$.ajax({
						url: '<?php echo $store_url; ?>index.php?routes=api/order/history&token=' + token + '&order_id=' + json['order_id'],
						type: 'post',
						dataType: 'json',
...
$('#revise-plan').on('click', function(e) {
	...
	$.ajax({
		url: 'index.php?routes=extension/payment/laybuy/revise&token=<?php echo $token; ?>&id=<?php echo $id; ?>&source=extension',
		type: 'post',
