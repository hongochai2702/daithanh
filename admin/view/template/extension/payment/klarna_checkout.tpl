...
$(document).on('click', '#button-process-settlement', function() {
	if (confirm('<?php echo $text_confirm_settlement; ?>')) {
		$.ajax({
			url: 'index.php?routes=extension/payment/klarna_checkout/downloadSettlementFiles&token=<?php echo $token; ?>',
			type: 'post',
			data: {
				...
				if (json['orders']) {
					...
					if (json['orders'].length > 0) {
						...
						$.each(json['orders'], function(index, value) {
							...
							$.ajax({
								url: '<?php echo $store_url; ?>index.php?routes=api/order/history&token=' + token + '&order_id=' + encodeURIComponent(value),
								type: 'post',
								dataType: 'json',
