...
$('#cancel-plan').on('click', function(e) {
	...
	$.ajax({
		url: 'index.php?routes=extension/payment/laybuy/cancel&token=<?php echo $token; ?>&id=<?php echo $id; ?>&source=order',
		type: 'post',
		dataType: 'json',
		cache: false,
		beforeSend: function() {
...
$('#revise-plan').on('click', function(e) {
	...
	$.ajax({
		url: 'index.php?routes=extension/payment/laybuy/revise&token=<?php echo $token; ?>&id=<?php echo $id; ?>&source=order',
		type: 'post',
