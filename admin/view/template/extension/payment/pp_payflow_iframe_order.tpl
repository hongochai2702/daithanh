...
function doVoid() {
    if (confirm('<?php echo $text_confirm_void; ?>')) {
        $.ajax({
            ...
            url: 'index.php?routes=extension/payment/pp_payflow_iframe/void&token=<?php echo $token; ?>',
            beforeSend: function() {
...
function capture() {
    ...
    $.ajax({
        ...
        url: 'index.php?routes=extension/payment/pp_payflow_iframe/capture&token=<?php echo $token; ?>',
