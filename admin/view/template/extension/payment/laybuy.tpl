...
<script type="text/javascript"><!--
	$('input[name="customer_group"]').autocomplete({
		source: function(request, response) {
			$.ajax({
				url: 'index.php?routes=extension/payment/laybuy/autocomplete&token=<?php echo $token; ?>&filter_customer_group=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
...
$('#button-filter').on('click', function() {
	url = 'index.php?routes=extension/payment/laybuy&token=<?php echo $token; ?>';
