...
class ModelDesignMenu extends Model {
	...
	public function addMenu($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "menu SET code = '" . $this->db->escape($data['code']) . "', name = '" . $this->db->escape($data['name']) . "', type = '" . $this->db->escape($data['type']) . "', picture = '" . $this->db->escape($data['picture']) . "', image = '" . $this->db->escape($data['image']) . "', status = '" . (int)$data['status'] . "'");
	...
	public function editMenu($menu_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "menu SET code = '" . $this->db->escape($data['code']) . "', name = '" . $this->db->escape($data['name']) . "', type = '" . $this->db->escape($data['type']) . "', picture = '" . $this->db->escape($data['picture']) . "', image = '" . $this->db->escape($data['image']) . "', status = '" . (int)$data['status'] . "' WHERE menu_id = '" . (int)$menu_id . "'");
	...
	public function groupMultipleAdd($data) {
		...
		if($data['module_type']=="blog"){
			foreach ($data['menu_blog'] as $module_id) {
				...
				$resultModule = $this->model_catalog_blog->getBlog($module_id);
				...
				$data['image'] = '';
				$data['style'] = '';
				$data['menu_group_languages'] = $this->model_catalog_product->getBlogDescriptions($module_id);
				$this->groupSingleAdd($data);
			}
		}
		...
		if($data['module_type']=="categoryblog"){
			foreach ($data['menu_categoryblog'] as $module_id) {
				$this->load->model('catalog/categoryblog');
				$resultModule = $this->model_catalog_categoryblog->getCategoryblog($module_id);
				$data['module_id'] = $resultModule['categoryblog_id'];
				$data['module_type'] = $data['module_type'];
				$data['url'] = 'categoryblog_id=' . $module_id;
				$data['keyword'] = '';
				$data['window'] = '0';
				$data['font'] = '';
				$data['image'] = '';
				$data['style'] = '';
				$data['menu_group_languages'] = $this->model_catalog_categoryblog->getCategoryblogDescriptions($module_id);
				$this->groupSingleAdd($data);
			}
		}
