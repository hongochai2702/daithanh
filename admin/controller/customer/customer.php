...
class ControllerCustomerCustomer extends Controller {
	...
	public function login() {
		...
		if ($customer_info) {
			...
			if ($store_info) {
				$this->response->redirect($store_info['url'] . 'index.php?routes=account/login&token=' . $token);
			} else {
				$this->response->redirect(HTTP_CATALOG . 'index.php?routes=account/login&token=' . $token);
			}
		} else {
