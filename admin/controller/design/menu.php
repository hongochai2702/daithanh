...
class ControllerDesignMenu extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('design/menu', 'token=' . $this->session->data['token'] . $urltrue));
		}
		$this->getForm();
	}
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['menus'][] = array(
				...
				'group'      => $this->url->link('design/menu/group', 'token=' . $this->session->data['token'] . '&menu_id=' . $result['menu_id'] . $url,true)
			);
		}
	...
	protected function validateForm() {
		...
		if(isset($this->request->post['code'])){
			if ((utf8_strlen($this->request->post['code']) < 1) || (utf8_strlen($this->request->post['code']) > 64)) { $this->error['code'] = $this->language->get('error_code'); }
		}
		if(isset($this->request->post['menu_group_description'])){
			foreach ($this->request->post['menu_group_description'] as $language_id => $value) {
				if ((utf8_strlen($value['name']) < 3) || (utf8_strlen($value['name']) > 255)) { $this->error['name'][$language_id] = $this->language->get('error_name'); }
			}
		}
	...
	public function groupSingleAdd() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('design/menu/group', 'token=' . $this->session->data['token'] . $url,true));
		}
		$this->group();
	}
	...
	protected function getDesign() {
		...
		$data['text_categoryblog'] = $this->language->get('text_categoryblog');
		...
		$data['groupSingleAdd'] = $this->url->link('design/menu/groupSingleAdd', 'token=' . $this->session->data['token'] . '&menu_id=' . $this->request->get['menu_id'],true);
		$data['groupMultipleAdd'] = $this->url->link('design/menu/groupMultipleAdd', 'token=' . $this->session->data['token'] . '&menu_id=' . $this->request->get['menu_id'],true);
		$data['cancel'] = $this->url->link('design/menu', 'token=' . $this->session->data['token'],true);
		$data['token'] = $this->session->data['token'];
		
		if (isset($this->request->get['menu_id']) ) {
		
			$data['menu_info'] = $this->model_design_menu->getMenu($this->request->get['menu_id']);
			$data['group_compose'] = $this->url->link('design/menu/groupCompose', 'token=' . $this->session->data['token'] . '&menu_id=' . $this->request->get['menu_id'],true);
			...
			if (isset($this->request->get['menu_group_id']) ) {
				...
				$data['groupSingleAdd'] = $this->url->link('design/menu/groupSingleEdit', 'token=' . $this->session->data['token'] . '&menu_id=' . $this->request->get['menu_id'] . '&menu_group_id=' . $this->request->get['menu_group_id'],true);
				$data['groupSingleCancel'] = $this->url->link('design/menu/group', 'token=' . $this->session->data['token'] . '&menu_id=' . $this->request->get['menu_id'],true);
			}else{
				$data['menu_group']  = '';
			}
		...
		$this->document->addStyle('view/stylesheet/menu/jquery-pg.nestable.css');
		$this->document->addStyle('view/stylesheet/menu/jquery-pg.iconpicker.css');
		$this->document->addStyle('view/javascript/font-awesome/css/font-awesome.css');
		
		$this->document->addScript('view/javascript/menu/jquery-pg.nestable.js');
		$this->document->addScript('view/javascript/menu/jquery-pg.iconpicker.js');
	...
	protected function contentParentHtml($menu_id, $parent_id) {
		...
		if( count($results) > 0 ){
			...
			foreach ($results as $result) {
				...
				$outrun .='<div class="dd-edit"><a href="'. $this->url->link('design/menu/groupSingleEdit', 'token=' . $this->session->data['token'] . '&menu_id=' . $this->request->get['menu_id'] . '&menu_group_id=' . $result['menu_group_id'],true) .'"><i class="fa fa-pencil"></i></a></div>';
				$outrun .='<div class="dd-delete">
					<a href="'.$this->url->link('design/menu/groupDelete', 'token=' . $this->session->data['token'] . '&menu_id=' . $this->request->get['menu_id'] . '&menu_group_id=' . $result['menu_group_id'],true).'" onclick="return delete_ms();">
	...
	public function species() {
		...
		/// custom blog/
		if($this->request->get['module_type'] == "categoryblog"){
			$this->load->model('catalog/categoryblog');
			$results = $this->model_catalog_categoryblog->getCategories();
			$output='';
			foreach ($results as $result) {
				$output .= '<option value="' . $result['categoryblog_id'] . '"';
				if ($this->request->get['module_id'] == $result['categoryblog_id']) { $output .= ' selected="selected"'; }
				$output .= '>' . $result['name'] . '</option>';
			}
			$this->response->setOutput($output);
		}
	...
	public function moduleurl() {
		...
		//custom blog//
		if($this->request->get['module_type'] == "categoryblog"){
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "categoryblog WHERE categoryblog_id = '" . (int)$this->request->get['module_id'] . "' ");
			$this->response->setOutput('categoryblog_id=' . $query->row['categoryblog_id']);
		}
		//end custom blog
		if($this->request->get['module_type'] == "product"){
